...
class FetchNflStandings extends Command
{
    /**
     * The name and signature of the console command.
     */
    protected $signature = 'nfl:fetch-standings
     ...
     */
    protected $description = 'Fetch NFL standings from API with caching support';
    ...
    {
        $this->info('🏈 Fetching NFL Standings...');
        ...
        try {
            ...
            if (!$this->option('force') && Cache::has($cacheKey)) {
                ...
                $standings = $this->fetchFromApi($options);

                if ($standings->isEmpty()) {
                    $this->error('❌ No standings returned from API');
                    return Command::FAILURE;
                }

                // Cache the results
                Cache::put($cacheKey, $standings, $cacheTtl);
                $fromCache = false;
            }
            ...
            /*     $this->storeInDatabase($standings); */
            /* } */
            ...
            Log::error('NFL fetch standings command failed', [
                'error' => $e->getMessage(),
    ...
    private function fetchFromApi(array $options): \Illuminate\Support\Collection
    {
        ...
        Log::info('NFL API fetch completed', [
            ...
            'season' => $data['standings']['category']['season'],
            'options' => $options
        ]);

        $standings = collect($data['standings']['category']['league'] ?? []);

        // Filter live games if requested
        if ($options['live_only']) {
            $standings = $standings->filter(function ($game) {
